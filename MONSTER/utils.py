...
from MONSTERimport CMD_LIST, LOAD_PLUG, LOGS, SUDO_LIST, bot
from MONSTERX import xbot
from MONSTER.helpers.exceptions import CancelProcess
...
def load_extra(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"MONSTER_PLUGS/{shortname}.py")
        name = "MONSTER_PLUGS.{}".format(shortname)
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = MONSTER.utils
        mod.Config = Config
        mod.borg = bot
        mod.edit_or_reply = edit_or_reply
        # support for paperplaneextended
        sys.modules["MONSTER.events"] = MONSTER.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["MONSTER.plugins." + shortname] = mod
        LOGS.info("Successfully imported " + shortname)
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"MONSTER/plugins/{shortname}.py")
        name = "MONSTER.plugins.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        LOGS.info("Successfully imported " + shortname)
...
def load_pro(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"MONSTER/plugins/assistant/{shortname}.py")
        name = "MONSTER.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        LOGS.info("Successfully imported " + shortname)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["MONSTER.plugins.assistant." + shortname] = mod
        LOGS.info("Successfully imported " + shortname)
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"MONSTER.plugins.{shortname}"
